    ...
    <div class="swiper-wrapper">
      <div class="swiper-slide">
        <div class="swiper-content">
          <div class="bg"></div>
          <div class="content">
            <img src="~assets/images/career/SketchPng8930def9a8d0537851a1a78d7a679434be13a282b9e5813d2566eccc88a89264.png" alt="">
            <p>{{ 11111 }}</p>
          </div>
        </div>
      </div>
...
```js
const records = [
  { 'sendCenterCode': '合计', 'typeName': '无维护路由', 'count': 305 },
  { 'sendCenterCode': '028GW3', 'typeName': '无维护路由', 'count': 287 },
  { 'sendCenterCode': '合计', 'typeName': '中心错建包', 'count': 456 },
  { 'sendCenterCode': '024GW0', 'typeName': '中心错建包', 'count': 252 },
  { 'sendCenterCode': '合计', 'typeName': '错装车', 'count': 9646 },
  { 'sendCenterCode': '024GW5', 'typeName': '错装车', 'count': 8286 },
  { 'sendCenterCode': '合计', 'typeName': '错建包', 'count': 254 },
  { 'sendCenterCode': '024GW5', 'typeName': '错建包', 'count': 75 },
  { 'sendCenterCode': '合计',  'typeName': '其他', 'count': 1966 },
]

const groupByTypeNameToArray = (records) => {
  const groupsMap =
    records.reduce((map, record) => {
      if (!map.has(record.typeName)) {
        map.set(record.typeName, {
          typeName: record.typeName,
          list: []
        })
      }
      map.get(record.typeName).list.push(record)
      return map
    }, new Map())
...
### 使用 `ECharts` 在vue中处理legend状态时进行缓存
